...
def get_features(models, unlabeled_loader):
    ...
    with torch.no_grad():
            for inputs, _, _, _ in unlabeled_loader:
                with torch.cuda.device(0):
                    inputs = inputs.cuda()
                    feat = models['backbone'](inputs)['l4']
                    feat = nn.functional.adaptive_avg_pool2d(feat, 4)
                    features_batch = feat.view(feat.size(0), -1)
