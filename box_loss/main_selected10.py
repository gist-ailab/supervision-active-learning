...
def train2(epoch, train_loader):
    ...
    for idx, (images, labels, heatmaps, img_id) in enumerate(pbar):
        ...
        # pred_hmap = F.interpolate(cam, size=(224,224))
        ...
        loss_hmap = heatmap_loss(pseudo_heatmaps, heatmaps)    
        if (idx+1)%10==0:
            alp = alp*0.9
...
def test2(epoch, best_acc, test_loader, mode):
    model.eval()
    test_loss = 0
    correct = 0
    total = 0
    with torch.no_grad():
        ...
        acc = 100*correct/total
        if acc > best_acc:
            torch.save({'model':model.state_dict(), 'h_model':heatmap_model.state_dict()}, os.path.join(save_path,f'{mode}_{epoch}_{acc:0.3f}_model.pt'))
            best_acc = acc
        return best_acc
...
if __name__ == "__main__":
    ...
    trainset = ilsvrc30(args.data_path, 'train', selected)
    train_subset = Subset(trainset, selected)
    valset = ilsvrc30(args.data_path, 'val', [])
    testset = ilsvrc30(args.data_path, 'test', [])
    
    train_loader = DataLoader(train_subset, args.batch_size, drop_last=True, shuffle=True, num_workers=2)
    val_loader = DataLoader(valset, args.batch_size, drop_last=True, shuffle=False, num_workers=2)
    ...
    for i in range(0, args.epoch2):
        ...
        best_acc = test2(i, best_acc, val_loader, 'tuning')
        model_scheduler.step()
    #------------------------------------------------------------------------------
    best_acc = 0
    para_list = os.listdir(save_path)
