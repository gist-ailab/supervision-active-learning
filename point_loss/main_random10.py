...
parser.add_argument('--save_path', type=str, default='/home/yunjae_heo/workspace/ailab_mat/Parameters/supervision/imagenet30/point_loss/random_1500')
...
def train2(epoch, train_loader):
    ...
    alp = 5
    pbar = tqdm(train_loader)
    img_aug = utils.img_aug_feature_sim()
    print(f'epoch : {epoch} _________________________________________________')
    for idx, (images, labels, heatmaps, img_id) in enumerate(pbar):
        images, labels, heatmaps = images.to(device), labels.to(device), heatmaps.to(device)
        # print(device)
        ...
        #-----------------------------------------------
        aug_imgs = img_aug(images, pred_hmap, heatmaps)
        outputs2, acts2 = model(aug_imgs)
        loss_hmap = 1-heatmap_loss(acts, acts2)    
...
if __name__ == "__main__":
    ...
    classif_loss = nn.CrossEntropyLoss()
    heatmap_loss = nn.CosineSimilarity(dim=-1)
